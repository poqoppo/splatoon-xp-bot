...
async def fetch_area_schedule(force=False):
    ...
    if (
        ...
        and (now - LAST_SCHEDULE_FETCH).total_seconds()
    ...
    async with API_LOCK:
        ...
        try:
            req = urllib.request.Request(
                ...
                headers={"User-Agent": "Splatoon3-XP-Bot/4.0"},
            )
            ...
            # Only replace cache after a successful parse.
            ...
            print(
                f"ガチエリアスケジュール更新: "
                f"{len(new_details)}件 / "
                f"次回更新まで{SCHEDULE_CACHE_SECONDS}秒"
            )

        except Exception as e:
            # Keep old cache on failure.
            print(f"Area API Fetch Error: {e}")
    ...
    async with ARCHIVE_LOCK:
        ...
        for season_name, recs in grouped.items():
            ...
            for rec in season_archived:
                ...
                # The local JSON is the source of truth. Discord
                # deletion is only cleanup of the audit copy.
                if log_channel and rec.get("_log_msg_id"):
                    ok = await safe_delete_message(
                        log_channel,
                        rec["_log_msg_id"],
                    )
                    if not ok:
