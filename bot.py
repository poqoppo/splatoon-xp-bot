import discord
from discord import app_commands  # ★スラッシュコマンド用に新規追加
import re
import os
import time
import math
...
class XPClient(discord.Client):
    ...
    async def setup_hook(self):
        # 起動時にDiscordへスラッシュコマンドを登録（同期）する
...
# コマンドの文字指定を解析する関数（スラッシュコマンド用に最適化）
...
@client.event
async def on_ready():
    print(f'{client.user} がスラッシュコマンドモードで起動しました！')
...
async def graph(interaction: discord.Interaction, 期間: str = None):
    await interaction.response.defer() # グラフ生成のロード時間確保
...
async def reset_last(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True) # 本人にだけ見えるメッセージで返信
