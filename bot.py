...
def get_graph_bounds(year_str, season_str, month_int):
    ...
    if season_str:
        if "春" in season_str:
            return (
                datetime(y, 3, 1, 0, 0, tzinfo=JST),
                datetime(y, 6, 1, 0, 0, tzinfo=JST),
            )

        if "夏" in season_str:
            return (
                datetime(y, 6, 1, 0, 0, tzinfo=JST),
                datetime(y, 9, 1, 0, 0, tzinfo=JST),
            )

        if "秋" in season_str:
            return (
                datetime(y, 9, 1, 0, 0, tzinfo=JST),
                datetime(y, 12, 1, 0, 0, tzinfo=JST),
            )

        if "冬" in season_str:
            return (
                datetime(y, 12, 1, 0, 0, tzinfo=JST),
                datetime(y + 1, 3, 1, 0, 0, tzinfo=JST),
            )
...
@client.tree.command(
    name="ログ件数",
    description="現在ログとアーカイブ済みログの件数を確認します"
)
...
@client.tree.command(
    name="手動アーカイブ",
    description="【管理者専用】現在ログを手動でアーカイブします"
)
@app_commands.describe(
...
async def manual_archive(interaction: discord.Interaction, 確認: str):
    ...
    if 確認 != "ARCHIVE":
        await interaction.followup.send(
            "⚠️ 確認文字列が違います。実行するには `ARCHIVE` と入力してください。"
        )
        return
    ...
    if success:
        await interaction.followup.send(
            f"📦 手動アーカイブ完了：**{count}件** を保存して現在ログから削除しました。"
        )

    else:
        await interaction.followup.send(
            f"⚠️ 手動アーカイブ失敗/未実行：{msg}（対象 {count}件）"
        )
...
async def graph(interaction: discord.Interaction, 期間: str = None):
    ...
    ty, ts, tm, ia, title, _ = parse_args_from_str(
        期間,
        season_year,
        current_season_type,
    )
    ...
    if not ia:
        if tm:
            recs = [
                r for r in recs
                if r["time"].year == int(ty) and r["time"].month == tm
            ]

        else:
            recs = [
                r for r in recs
                if r["season"] == f"{ty}年 {ts}"
            ]
...
async def comp_graph(
    ...
    now = datetime.now(JST)
    season_year, current_season_type = get_current_season(now)
    ty, ts, tm, ia, title, is_continuous = parse_args_from_str(
        期間,
        season_year,
        current_season_type,
    )
...
async def delete_my_data(interaction: discord.Interaction, 確認: str):
    ...
    if 確認 != "DELETE":
        await interaction.followup.send(
            "⚠️ 確認文字列が違います。削除するには `DELETE` と入力してください。"
        )
        return
...
async def delete_member_data(
    ...
    if 確認 != "RESET":
        await interaction.followup.send(
            "⚠️ 確認文字列が違います。削除するには `RESET` と入力してください。"
        )
        return
...
async def reset_all_data(interaction: discord.Interaction, 確認: str):
    ...
    if 確認 != "RESET":
        await interaction.followup.send(
            "⚠️ 確認文字列が違います。全削除するには `RESET` と入力してください。"
        )
        return
...
async def on_message(message):
    ...
    for uid, info in all_d.items():
        s_recs = [
            r for r in info["records"]
            if r["season"] == curr_season_full_str
        ]
    ...
    if BOT_SETTINGS.get("drama_enabled", True):
        ...
        if my_index == 0:
            ...
            if diff_above == 0:
                ...
                drama_msg += (
                    f"\n🎯 1つ上の{above_name}さんまであと "
                    f"**XP {diff_above}**！一歩ずつ距離を詰めよう！"
                )
    ...
    success, archived_count, archive_msg = await auto_archive_if_needed(
        force=False
    )
            ...
            if spec_time:
                ...
                if target_channel:
                    ...
                    time_range_str = (
                        ...
                        f"ー{spec_time.strftime('%H:%M')}"
                    )
                    await target_channel.send(
                        f"🔄 記録枠を **{time_range_str}** に変更しました！"
                    )
